...
class Potok_parser:
    ...
    def table_drawer(self):        
        ...
        if self.download_mode == False:
            df['Дата'] = df['Дата'].str[:5]
            res = df.rename(columns={"Группа": "Гр."}).drop('Тип', axis = 1).to_string(index = False, justify='center', col_space = 10) 
            return res
        else:
...
#Potok_parser("Дневная", "Курс 1", "На неделю", "ПИГС").parse()
